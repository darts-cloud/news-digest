...
def main() -> int:
    ...
    # JST基準で「前日（ニュース取得対象日）」を計算
    target_date_jst = (datetime.datetime.now() - datetime.timedelta(days=1)).strftime('%Y-%m-%d')
    prompt_cmd = f"/news-digest '{topic}' '{target_date_jst}'"
    ...
    print(f"Target News Date (JST): {target_date_jst}")
    print(f"Model: {model}")
